...
public static class ExpressionConverterExtension
{
    public static Expression<Func<TEntity, TProperty>> TranslateFromModelToEntity<TEntity, TModel, TProperty>(
        this Expression<Func<TModel, TProperty>> expression,
        IPropertyMapper<TEntity, TModel> propertyMapper)
    {
        var expressionConverter = new ExpressionConverter<TEntity, TModel>(propertyMapper);
        return expressionConverter.TranslateFromModelToEntity(expression);
    }

    private class ExpressionConverter<TEntity, TModel>
    {
        ...
        public Expression<Func<TEntity, TProperty>> TranslateFromModelToEntity<TProperty>(Expression<Func<TModel, TProperty>> expression)
        {
